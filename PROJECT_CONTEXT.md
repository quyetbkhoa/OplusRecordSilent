...
4. **Tự động đặt tên file ghi âm chuẩn**: Định dạng tên file thành `[AppName]_[CallerName]_[Timestamp].aac` (ví dụ: `Messenger_Bố_2026-09-19_18-45-00.aac`) và đồng bộ ngay lập tức vào ứng dụng Ghi âm hệ thống (`com.coloros.soundrecorder`).
...
- **4 lớp can thiệp trong LSPosed (`XposedInit.java`)**:
  - **Layer 1**: Hook `com.coloros.translate.engine.mixaudio.a.e` - Chặn phát âm thanh và giả lập callback `onPlayComplete` để hệ điều hành tiếp tục quy trình ghi âm mà không bị treo.
  - **Layer 2**: Hook `com.coloros.translate.engine.mixaudio.mix.a.e` - Chặn worker player trực tiếp.
  - **Layer 3**: Hook `com.coloros.translate.engine.mixaudio.mix.c.d` - Chặn luồng Downlink PCM stream.
  - **Layer 4**: Hook `AudioTrack.play()` - Lưới an toàn lọc Call Stack chặn mọi lệnh `play()` từ các class âm thanh thông báo thuộc `mixaudio` / `translate.engine`.
- **Tầng KernelSU (`service.sh`)**:
  ...
  - Dùng `mount -o bind` đè APK này lên `/product/app/ColorAccessibilityAssistant/ColorAccessibilityAssistant.apk` trên cả global namespace và toàn bộ namespace của `zygote64`/`zygote`.
...
### D. Tự động đổi tên file & đồng bộ Sound Recorder
- **Tầng LSPosed**: Hook `GlobalSummaryInfo.getFormatSaveFileName` kết hợp cùng `UserNameNotificationListenerService` bắt tên người gọi từ thông báo VoIP đang đàm thoại để tạo tên file:
  `[AppName]_[CallerName]_[YYYY-MM-DD_HH-mm-ss].aac`.
- **Tầng KernelSU (Daemon Fallback)**: Tiến trình daemon chạy ngầm trong `service.sh` theo dõi thư mục `/storage/emulated/0/Music/Recordings/Call Recordings/`. Khi phát hiện file mới, daemon truy vấn `dumpsys notification` và kích hoạt broadcast `MEDIA_SCANNER_SCAN_FILE` để bản ghi âm hiển thị tức thì trong ứng dụng Ghi âm hệ thống.
...
1. **Trạng thái**:
   - Toàn bộ 18 hook trong `SilentAICall.apk` đã được nạp thành công vào `com.coloros.accessibilityassistant`.
   - Ứng dụng quản lý `org.lsposed.manager` đã được cài đặt và hoạt động bình thường.
2. **Kế hoạch kiểm tra nghiệm thu**:
   ...
   - Kiểm tra các tiêu chí:
     ...
     3. Tên file định dạng đúng: `[AppName]_[CallerName]_[Timestamp].aac`.
     4. Banner / thông báo "Đã lưu bản ghi âm" xuất hiện trên màn hình.
     5. Bản ghi âm xuất hiện trong danh sách của ứng dụng Ghi âm (Sound Recorder).
